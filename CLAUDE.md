...
**El bloque ya está pegado en 5 páginas más que todavía no están vinculadas como Clase en el panel** (código listo, falta el paso de ATENZA): `guias_bd/dml.html` (17/08 — reemplazó al bloque viejo de 05/08 sin nota/sin pantalla completa, que ya no existe en el archivo), `guias/taller-funciones.html` (18/08), `guias_bd/taller-joins-subconsultas.html` (19/08), `guias_bd/taller-dml.html` (17/08), `guias/taller-clases-objetos.html` (24/08). En los tres talleres de BD2 el bloque solo reporta la sección "Repaso"/preguntas con `data-quiz-id` — los ejercicios sueltos con editor SQL no reportan nada a ATENZA, a propósito. `taller-clases-objetos.html` no tiene esa distinción: en POO **todos** los `editor-panel` (los 10 ejercicios + el reto final) ya traen `data-quiz-id`, igual que `taller-funciones.html`.

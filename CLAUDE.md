...
**Al redactar `quiz-mc` (opción múltiple y verdadero/falso): nunca dejar pistas involuntarias en la forma de la pregunta.** Dos patrones concretos a vigilar en cada tanda de opciones antes de darla por terminada:
- **Extensión de las opciones**: si la correcta es sistemáticamente la más larga/detallada (o la más corta) mientras los distractores son escuetos, el estudiante puede acertar por longitud sin razonar el contenido. Las 3-4 opciones de una misma pregunta deben quedar en un rango de longitud comparable — recortar la correcta o desarrollar los distractores, lo que haga falta.
- **Posición de la respuesta correcta**: no debe repetirse la misma posición (el mismo inciso) pregunta tras pregunta dentro de una guía. Al terminar de escribir todos los `quiz-mc` de una guía, repasar en qué posición (1º, 2º, 3º, 4º) quedó `data-correct` en cada una y redistribuir si se nota un patrón (por ejemplo, "casi siempre la segunda opción"). Mismo cuidado con los `<select>` de `quiz-fill`: no dejar la opción correcta siempre primera (o siempre última) en la lista tras el placeholder "— elige —".
`quiz-match` no tiene este problema (el orden de la columna derecha ya se mezcla al azar por JS) y `quiz-classify` reparte naturalmente la posición correcta entre sus botones porque cada fila tiene su propia respuesta distinta — pero igual conviene repasarlo.

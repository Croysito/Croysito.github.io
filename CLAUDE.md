...
**El bloque ya está pegado en 6 páginas más que todavía no están vinculadas como Clase en el panel** (código listo, falta el paso de ATENZA): `guias_bd/dml.html` (17/08 — reemplazó al bloque viejo de 05/08 sin nota/sin pantalla completa, que ya no existe en el archivo), `guias/taller-funciones.html` (18/08), `guias_bd/taller-joins-subconsultas.html` (19/08), `guias_bd/taller-dml.html` (17/08), `guias/taller-clases-objetos.html` (24/08), `guias_tgs/sistema.html` (25/08). En los tres talleres de BD2 el bloque solo reporta la sección "Repaso"/preguntas con `data-quiz-id` — los ejercicios sueltos con editor SQL no reportan nada a ATENZA, a propósito. `taller-clases-objetos.html` no tiene esa distinción: en POO **todos** los `editor-panel` (los 10 ejercicios + el reto final) ya traen `data-quiz-id`, igual que `taller-funciones.html`. `sistema.html` es la única de las 6 que ya nace con `paramsDeAtenza()` (busca los parámetros también en `location.hash`) en vez de `new URLSearchParams(location.search)` a secas — mitiga desde el día uno el bug de URL descrito más abajo; las otras 5 todavía lo necesitan.

**Nota de auditoría (25/08, `sistema.html`):** al revisar esta guía contra el roadmap de TGS se encontró y corrigió un patrón posicional no intencional en sus actividades: los 7 `quiz-mc` de 4 opciones tenían la respuesta correcta en las posiciones 2,3,4,1,2,3,4 (ciclo ascendente perfecto), y el `quiz-classify` de Entrada/Proceso/Salida repetía el patrón diagonal fila→posición (1,2,3,1,2,3) que advierte la sección de auditoría de quizzes más abajo en este documento. También una pregunta (`jerarquia-1`) tenía razón largo_máx/largo_mín de 6.27 por un distractor mucho más largo que el resto. Las tres cosas se corrigieron reordenando opciones/filas (sin tocar `data-correct` semánticamente) y acortando/parejando textos; verificado con script, no a ojo — mismo criterio que `joins-subconsultas.html`.

**Nota (25/08):** `guias_tgs/fundamentos-origen.html` ya tiene un bloque de ATENZA en el archivo (con una variante más avanzada del motor: limpieza de intento en el `<head>`, hooks `window.atenzaReportarAutomatica`/`atenzaFinalizarGuia` integrados directo en `marcarResuelto()`/`actualizarProgreso()`, y semántica "sin reintentos" en match/classify) que no está documentada en ninguna de las dos listas de arriba ("vinculadas hoy" / "bloque pegado, falta vincular"). No se tocó al auditar `sistema.html` (fuera de alcance de ese pedido) pero conviene que alguien concilie esa página con esta documentación la próxima vez que se edite.
